...
for i in range(len(year)):
    ...
    tb.append(
        [
            (year[i] - 1850) * 365 + bnd_months[month[i] - 1],
...
for cf_name, var_name in zip(["gpp", "hfss", "hfls"], ["GPP", "H", "LE"]):
...
@ARTICLE{Alemohammad2017,
  ...
  doi = {https://doi.org/10.5194/bg-14-4101-2017}
        ...
        oset.comments = """
time_period: %d-%02d through %d-%02d; temporal_resolution: monthly; spatial_resolution: %.1f degree; units: %s""" % (
            year[0],
            month[0],
            year[-1],
            month[-2],
